...
The application has not been tested for Internet Explorer or other browser versions, so that some parts may not work properly 
or the webinterface differs from the pictures. In order to access the API it is necessary to interpret JSON object corretly.


\subsection{Backend}
The application backend can be executed an Windows and Linux applicatoin server. 
...
entry point for the webapplicatoin.
